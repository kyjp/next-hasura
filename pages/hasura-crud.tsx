  ...
  return (
    <Layout title="hasura CRUD">
      ...
      >
        ...
        />
        <button type="submit" className="disabled:opacity-40 my-3 py-1 px-3 text-white bg-indigo-600 hover:bg-indigo-700 rounded-2xl focus:outline-none" data-testid="new" disabled={!editedUser.name}>{editedUser.id ? 'Update' : 'Create'}</button>
      </form>
      {data?.users.map(user => {
        return (
          <UserItem
